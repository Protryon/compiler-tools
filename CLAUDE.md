...
Supported syntax: literals, `[...]` classes and `a-z` ranges, negation `[^...]`, `.` (any char except `\n`, like the `regex` crate), the quantifiers `* + ?` and counted `{n} {n,} {n,m}` (each in a greedy or lazy `*? +? ?? {n,m}?` form), control-char escapes (`\n \t \r \0 \f \v`), hex/codepoint escapes (`\x41`, `\uXXXX`, `\UXXXXXXXX`, and braced `\x{..}`/`\u{..}`/`\U{..}`), ASCII Perl shorthand classes (`\d \D \w \W \s \S`), Unicode property classes `\p{..}`/`\P{..}` (and the `\pL` shorthand; resolved to codepoint ranges at build time via `regex-syntax` in `unicode.rs`, usable top-level and inside `[...]`), a leading `^`/`\A` (no-op for a prefix matcher), a trailing `$`/`\z` (end-of-input assertion, lowered to `Atom::EndOfInput`/`TransitionEvent::EndOfInput`), and ASCII word boundaries `\b`/`\B` (`Atom::WordBoundary`), and `(?m)` multiline `^`/`$` (`Atom::StartOfLine`/`EndOfLine`: `^` holds at start-of-input or after a `\n`, `$` at end-of-input or before a `\n`, and they may appear anywhere/quantified). The word-boundary/anchor zero-width assertions are evaluated in the generated matcher loop, which holds a lookahead char + `prev` and only advances on a consuming match (so `MatchedEmpty` transitions are truly zero-width). **Alternation `a|b` and grouping `(...)`** are supported, including nesting, a quantifier on a group (`(ab)+`), and non-capturing/named groups (`(?:...)`, `(?P<n>...)`, `(?<n>...)` — all treated identically since this engine never extracts capture spans). **Inline flags / modes** are supported — both the bare `(?flags)` directive (applies to the rest of the enclosing group) and the scoped `(?flags:...)` group, with `-` clearing: `i` (case-insensitive — a cased literal becomes a multi-case group and class members/ranges are folded; ASCII folding by default, full Unicode simple case folding under `(?iu)` via `regex-syntax`, see `unicode::case_fold`), `s` (dot-all — `.` lowers to an inverted empty class), `x` (verbose — drop unescaped whitespace + `#` comments outside `[...]`), `U` (swap-greedy), and `u` (Unicode toggle — sets a `unicode` flag, defaulting *off* so `#[token(regex = ...)]` stays ASCII; it does not yet retarget `\d \w \s`/`\b` to Unicode definitions). `m` (multiline — `^`/`$` become `\n` line anchors, see above). See `Flags`/`apply_flag`/`fold_entry` in `parse.rs`. Lookaround and the `R` (CRLF) flag are rejected at parse time — the line-anchor model is `\n`-only and can't yet treat `\r\n` as one terminator. Still missing: POSIX classes, Unicode-aware `\d \w \s`/`\b` (the ASCII forms are used even under `(?u)`), octal escapes, CRLF/custom line terminators — use `regex_full` for those. `REGEX_PARITY.md` tracks the remaining gaps against the `regex` crate.
